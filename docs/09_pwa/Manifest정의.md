...
| ----- | ------------------- |
| 필드    | `theme_color`       |
| 값     | `#fda5e3` (기본값, pink 테마) |
| 필수 여부 | 권장                  |
| 용도    | 브라우저 UI 및 시스템 테마 색상 |

정책

* Manifest의 `theme_color`는 pink 테마 기본값(`#fda5e3`)으로 설정한다.
* 런타임에는 `applyThemeSetting()`이 `<meta name="theme-color">` 값을 현재 테마에 맞게 동적으로 업데이트한다.
* 테마별 색상 매핑은 `src/entities/settings/model/theme.ts`의 `THEME_PRIMARY_COLORS`에서 관리한다.
...
| 512x512 maskable | `maskable-icon-512x512.png`    | Adaptive Icon 대응 |
| 32x32            | `favicon-32x32.png`            | 브라우저 탭 파비콘       |
| 16x16            | `favicon-16x16.png`            | 브라우저 탭 파비콘 (소)   |
| ico              | `favicon.ico`                  | 레거시 브라우저 파비콘     |
| 180x180          | `apple-touch-icon.png`         | iOS 홈 화면 아이콘     |

아이콘은 현재 선택된 테마에 따라 동적으로 교체된다. `applyThemeSetting()` 호출 시 `<link>` 태그의 `href`가 해당 테마 폴더 경로로 업데이트된다.
...
파비콘 및 Apple Touch 아이콘은 `index.html`에 정적 링크로 선언하고, 테마 변경 시 `applyThemeSetting()`에서 동적으로 교체한다.

```html
<!-- index.html -->
<link id="favicon-ico" rel="icon" type="image/x-icon" href="/kittywallet/icons/themes/pink/favicon/favicon.ico" />
<link id="favicon-32" rel="icon" type="image/png" sizes="32x32" href="/kittywallet/icons/themes/pink/favicon/favicon-32x32.png" />
<link id="favicon-16" rel="icon" type="image/png" sizes="16x16" href="/kittywallet/icons/themes/pink/favicon/favicon-16x16.png" />
<link id="apple-touch-icon" rel="apple-touch-icon" href="/kittywallet/icons/themes/pink/apple-touch/apple-touch-icon.png" />
```
