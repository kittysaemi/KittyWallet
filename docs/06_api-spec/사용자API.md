...
- 사용자별 데이터 완전 분리
- 탈퇴 시 사용자 등록 데이터 물리 삭제 처리
- 탈퇴 후 동일 이메일로 즉시 재가입 가능
- 탈퇴 후 기존 데이터 복구 불가
- 인증 사용자만 접근 가능
...
- 404 응답은 로컬 인증 상태와 사용자 캐시를 제거하고 재로그인을 요구한다.
...
| 항목 | 처리 기준 |
|---|---|
| 삭제 방식 | 사용자 등록 데이터 물리 삭제 (단일 DB transaction) |
| 삭제 순서 | SYNC_HISTORY → TRANSACTION → CATEGORY_USER_SETTING → USER_SETTING → ACCOUNT → CARD → 사용자 생성 CATEGORY → 사용자 생성 ICON → SYNC_CLIENT → REFRESH_TOKEN → USER |
| 공통 seed 데이터 | 삭제하지 않음 |
| 로그/백업 | 삭제 범위에서 제외 |
| 토큰 처리 | Access Token과 Refresh Token 모두 폐기 |
| 재가입 | 탈퇴 직후 동일 이메일로 즉시 신규 가입 가능 |
| 데이터 복구 | 탈퇴 후 기존 데이터 복구 불가 |
