...
| 최대 원본 | 25MB |
| 정규화 | EXIF 회전 보정 → 카메라 여부 판단(EXIF 존재, HEIC는 항상 카메라) → 스크린샷 한정 배경 제거 → 다크 배경 반전(해당하는 경우) → JPEG 변환, 긴 변 2,000px 이하, 최대 5MB |
