  ...
  await ctx.route("**/api/v1/transactions**", async (route) => {
    ...
    if (path.endsWith("/transactions/transfer") && method === "POST") {
      ...
      await fulfillJson(route, 201, success({
        ...
        from_account_name: accountNameById.get(body.from_account_id) ?? "",
        from_account_deleted: false,
        to_account_id: body.to_account_id,
        to_account_name: accountNameById.get(body.to_account_id) ?? "",
    ...
    if (transferMatch) {
      ...
      if (method === "GET") {
        ...
        await fulfillJson(route, 200, success({
          ...
          from_account_name: accountNameById.get(from.wallet_id) ?? "",
          from_account_deleted: false,
          to_account_id: to.wallet_id,
          to_account_name: accountNameById.get(to.wallet_id) ?? "",
          to_account_deleted: false,
          amount: from.amount,
      ...
      if (method === "PATCH") {
        ...
        await fulfillJson(route, 200, success({
          ...
          from_account_name: accountNameById.get(fromAccountId) ?? "",
          from_account_deleted: false,
          to_account_id: toAccountId,
          to_account_name: accountNameById.get(toAccountId) ?? "",
          to_account_deleted: false,
          amount,
