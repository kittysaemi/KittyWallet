...
test("PWA: manifest.webmanifest 내용 확인", async ({ page }) => {
  const res = await page.request.get("/kittywallet/manifest.webmanifest");
...
test("PWA: 설치 배너 — installable 상태에서 표시, 닫기 후 숨김 및 localStorage 기록", async ({
  ...
  await page.goto("/kittywallet/dashboard");
  ...
  await expect(page.locator("text=KittyWallet을 홈 화면에 설치해보세요.")).toBeVisible({
    timeout: 3000
  });
