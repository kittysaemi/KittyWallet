import React from "react";
import { useNavigate, useParams } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import { cardApi } from "../../../entities/card/api/cardApi";
import { CardForm } from "../../../features/cards/CardForm";
import { Button } from "../../../shared/ui/Button";

const CardEditPage: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const cardId = Number(id);

  const cardsQuery = useQuery({
    queryKey: ["cards"],
    queryFn: () => cardApi.getCards()
  });

  const card = cardsQuery.data?.data?.items.find((c) => c.card_id === cardId);

  if (cardsQuery.isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--color-bg-primary)]">
        <p className="text-sm text-[var(--color-text-secondary)]">불러오는 중...</p>
      </div>
    );
  }

  if (!card) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[var(--color-bg-primary)] px-4">
        <p className="text-[var(--color-text-secondary)]">카드를 찾을 수 없습니다.</p>
        <Button type="button" variant="secondary" onClick={() => navigate("/cards")}>
          목록으로
        </Button>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] px-4 py-6">
      <div className="mx-auto flex w-full max-w-[480px] flex-col gap-6">
        <header className="flex items-center gap-3">
          <Button type="button" variant="ghost" onClick={() => navigate("/cards")}>←</Button>
          <h1 className="text-xl font-bold text-[var(--color-text-primary)]">카드 수정</h1>
        </header>
        <CardForm mode="edit" card={card} />
      </div>
    </div>
  );
};

export default CardEditPage;
