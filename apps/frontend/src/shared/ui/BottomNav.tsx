...
export const BottomNav: React.FC = () => {
  ...
  const webCameraSource = React.useMemo(() => new WebCameraReceiptImageSource(), []);
  const filePickerSource = React.useMemo(() => new FilePickerReceiptImageSource(), []);

  React.useEffect(() => {
    webCameraSource.isAvailable().then(setCameraAvailable);
  }, [webCameraSource]);
  ...
  const handleCameraClick = async () => {
    try {
      const file = await webCameraSource.acquire();
      setIsEntrySheetOpen(false);
      navigate("/transactions/new?receiptSource=camera", { state: { receiptFile: file } });
    } catch {
      // CANCELLED or NO_FILE — keep sheet open, do nothing
    }
  };

  const handleGalleryClick = async () => {
    try {
      const file = await filePickerSource.acquire();
      setIsEntrySheetOpen(false);
      navigate("/transactions/new", { state: { receiptFile: file } });
    } catch {
      // CANCELLED or NO_FILE — keep sheet open, do nothing
    }
  };
          ...
          >
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <span className="text-sm font-semibold text-[var(--color-text-primary)]">직접 입력</span>
              </button>
              {cameraAvailable && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-[var(--color-text-primary)]">카메라 입력</span>
                </button>
              )}
              <button
                type="button"
                ...
                className="flex min-h-14 w-full items-center gap-3 rounded-2xl border border-[var(--color-border-primary)] px-4 text-left hover:bg-[var(--color-bg-secondary)]"
              >
                <FileImage size={20} className="text-[var(--color-primary)]" />
                <span className="text-sm font-semibold text-[var(--color-text-primary)]">사진 입력</span>
              </button>
              <button
                type="button"
                onClick={() => startTransaction("text")}
                className="flex min-h-14 w-full items-center gap-3 rounded-2xl border border-[var(--color-border-primary)] px-4 text-left hover:bg-[var(--color-bg-secondary)]"
              >
