...
describe("ManifestController", () => {
  ...
  it("returns pink manifest by default", () => {
    const res = mockRes();
    controller.getManifest(undefined as unknown as string, res as never);
  ...
  it("returns mint manifest for theme=mint", () => {
    const res = mockRes();
    controller.getManifest("mint", res as never);
    const manifest = (res.json as jest.Mock).mock.calls[0][0];
  ...
  it("returns purple manifest for theme=purple", () => {
    const res = mockRes();
    controller.getManifest("purple", res as never);
    const manifest = (res.json as jest.Mock).mock.calls[0][0];
  ...
  it("falls back to pink for unknown theme", () => {
    const res = mockRes();
    controller.getManifest("unknown", res as never);
    const manifest = (res.json as jest.Mock).mock.calls[0][0];
    expect(manifest.theme_color).toBe("#fda5e3");
    expect(manifest.icons[0].src).toContain("/themes/pink/pwa/");
  });
  ...
  it("includes required manifest fields", () => {
    const res = mockRes();
    controller.getManifest("pink", res as never);
    const manifest = (res.json as jest.Mock).mock.calls[0][0];
