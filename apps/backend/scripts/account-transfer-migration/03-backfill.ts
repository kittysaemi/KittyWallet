...
async function main() {
  ...
  try {
    ...
    for (const m of approved) {
      ...
      >(
        `SELECT transaction_id, deleted_yn, amount, wallet_id, ${columnName} AS transfer_group_id
        ...
        if (!opts.skipMismatches) {
          throw new Error(
            `승인 파일과 현재 DB 상태가 다릅니다 (${msg}). ` +
              `안전을 위해 실행을 중단합니다. 문제를 확인한 뒤 다시 실행하거나, ` +
              `의도적으로 건너뛰려면 --skip-mismatches를 사용하세요.`
          );
        }
        continue;
      }
    ...
    console.log(
      "\n다음 단계: 04-snapshot.ts label=after 로 스냅샷을 뜨고, " +
        "05-compare-snapshots.ts로 백필 전(before) 스냅샷과 비교해 무결성을 검증하세요."
    );
  } finally {
...
function validatePairAgainstCurrentState(
  ...
  current: { transaction_id: bigint; deleted_yn: boolean; amount: unknown; wallet_id: bigint; transfer_group_id: string | null }[]
): string[] {
